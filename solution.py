...
def smtp_client(port=1025, mailserver='127.0.0.1'):
	...
	# Fill in start
	data_cmd = 'something for data command'
	print data_cmd
	Client_SMPT_Socket.send(data_cmd.encode())
	recv1 = clientSocket.recv(1024)
	print recv1
	if recv1[:3] != '250':
		print('250 reply not received from server.')
	...
	# Send message data.
	# Fill in start
	msg = raw_input('Enter Message Here: ')
	# Fill in end

	# Message ends with a single period.
	# Fill in start
	mail_msg_end = ‘\r\n.\r\n’
	Client_SMPT_Socket.send((msg+ mail_msg_end).encode()
	print recv1
	if recv1[:3] != '250':
		print('250 reply not received from server.')
	# Fill in end
